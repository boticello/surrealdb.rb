...
RSpec.describe SurrealDB::GeometryLine do
  let(:p1) { SurrealDB::GeometryPoint.new(0.0, 0.0) }
  let(:p2) { SurrealDB::GeometryPoint.new(1.0, 1.0) }
  let(:p3) { SurrealDB::GeometryPoint.new(2.0, 0.0) }

  it "requires at least 2 points" do
    expect { described_class.new(p1) }.to raise_error(ArgumentError)
  end

  it "stores points" do
    line = described_class.new(p1, p2, p3)
    expect(line.points.length).to eq(3)
  end

  it "returns coordinates" do
    line = described_class.new(p1, p2)
